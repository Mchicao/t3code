...
it.effect("marks only strict imported bindings as already imported", () =>
  Effect.gen(function* () {
...
it.effect("imports each selected native session once with a strict continuation binding", () =>
  Effect.gen(function* () {
    ...
    const source: ProviderThreadHistorySource = {
      listThreads: () =>
        Effect.succeed({ threads: [candidate("native-session")], truncated: false }),
    ...
    expect(dispatches[0]).toMatchObject({
      ...
      messages: [
        expect.objectContaining({
          messageId: MessageId.make("codex:codex:native-session:message:message-1"),
          role: "user",
          text: "Please continue this work.",
          turnId: null,
        }),
...
it.effect("refuses a stale selection before reading or creating any T3 thread", () =>
  Effect.gen(function* () {
    let readCount = 0;
    const dispatches: OrchestrationCommand[] = [];
    const upserts: ProviderSessionDirectory.ProviderRuntimeBinding[] = [];
